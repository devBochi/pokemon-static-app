...
export const getStaticPaths: GetStaticPaths = async (ctx) => {
  ...
  const pokemons151 = await pokeApi.get<PokemonListResponse>(`/pokemon?limit=151`)
  const pokeList: string[] = pokemons151.data.results.map(pokemon => pokemon.name)
  
  return {
    paths: pokeList.map( name => ({
      params: {name}
    })),
    fallback: 'blocking'
  }
